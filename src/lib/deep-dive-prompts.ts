// ─── Deep Dive — Podcast Script Prompt ──────────────────────────────────────
// Generates a Pamela + Brian dialogue podcast about the user's Invisible Rule.
// Pamela is the lead voice — warm, direct, precise. Brian is the thoughtful
...
Write exactly 8 segments — alternating PAMELA, BRIAN, PAMELA, BRIAN, PAMELA, BRIAN, PAMELA, BRIAN.

Guidelines:
- Each segment: 60–90 words. Conversational. No clinical language.
...
- Segment 3 (Pamela): Speak to where the rule came from — the origin. Validate the intelligence of the child who made it.
- Segment 4 (Brian): Speak to what the rule protected. Name it with warmth and respect.
- Segment 5 (Pamela): Turn to what it costs today. Be direct but compassionate — name what keeps getting lost.
- Segment 6 (Brian): Speak to the evolved principle — what ${data.firstName || 'they'} is building now. Make it feel possible.
...
// ─── Deep Dive — Video Narration Prompt ──────────────────────────────────────
// Generates Pamela's solo narration script for the personalized 3-minute video.
// 8 segments × ~55 words = ~440 words = ~3 minutes at natural speaking pace.
...
Pamela speaks directly to the viewer — warm, precise, deeply personal. She knows this person's story. She never uses filler phrases, never hedges. Every sentence lands.
...
Guidelines:
- Each segment: 50–60 words. Direct. No jargon. No hedging. Speaks to this specific person.
- Segment 1: Address ${data.firstName || 'them'} by name. State their Invisible Rule clearly. Say "This is yours."
- Segment 2: Speak to the origin — the moment or environment that made this rule feel necessary. Honor the child who created it.
- Segment 3: Name what the rule protected. Speak to it with respect — it was smart at the time.
- Segment 4: Turn to the cost. Name specifically what keeps getting lost or limited because of this rule.
- Segment 5: Speak to the archetype — the pattern name and what it reveals about how they've been moving through life.
- Segment 6: Introduce the evolved principle. Name the new operating system they are building.
- Segment 7: Give one specific thing to do in the next 7 days — small, concrete, personal to their pattern.
- Segment 8: Close the video. Affirm the courage it took to do this work. End with one sentence that stays with them.

Return as a JSON array of 8 strings — just the spoken text, no speaker labels:
["segment 1 text", "segment 2 text", ..., "segment 8 text"]

No other output. Just the JSON array.
`;

// ─── Deep Dive — Slide Deck Prompt ──────────────────────────────────────────
...
- "slide": slide number (1–8)
- "label": small caps section label (e.g. "Your Invisible Rule", "The Origin", "The Cost")
- "headline": 1 powerful sentence — the main point of this slide (15–25 words)
- "body": 2–3 sentences of elaboration (40–60 words total) — specific and personal
- "type": one of: "cover" | "rule" | "insight" | "closing"

Slide structure:
1. Cover — type: "cover" — Welcome ${data.firstName || 'the participant'} and frame the journey
2. The Rule — type: "rule" — State the Invisible Rule in full; honor it
3. The Origin — type: "insight" — Where the rule was formed; validate the child who made it
4. What It Protected — type: "insight" — What the rule kept safe; name the payoff with warmth
5. What It Costs Today — type: "insight" — What the rule keeps limiting in adult life; name it directly
6. The Pattern — type: "insight" — The repeating theme across tolerations and life areas
7. Your Evolved Principle — type: "insight" — The new operating principle being built
8. Your Path Forward — type: "closing" — The first step; the invitation into the work
